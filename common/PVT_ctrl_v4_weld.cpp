/*
 * PVT_Ctr_V4_Weld implementation for speedbot_v4_weld robot.
 * Adapted from PVT_ctrl.cpp for OpenLoong.
 */
#include "PVT_ctrl_v4_weld.h"

PVT_Ctr_V4_Weld::PVT_Ctr_V4_Weld(double timeStepIn, const char *jsonPath) {
...
void PVT_Ctr_V4_Weld::dataBusRead(DataBus &busIn) {
    for (int i=0;i<jointNum;i++)
    {
...
void PVT_Ctr_V4_Weld::dataBusWrite(DataBus &busIn) {
    busIn.motors_tor_out=motor_tor_out_motor;
    busIn.motors_tor_cur=motor_tor_out_link;
}

void PVT_Ctr_V4_Weld::setJointPD(double kp, double kd, const char *jointName) {
...
void PVT_Ctr_V4_Weld::calMotorsPVT() {
    for (int i=0;i<jointNum;i++)
    {
        double tauDes{0};
...
void PVT_Ctr_V4_Weld::calMotorsPVT(double deltaP_Lim) {
    for (int i=0;i<jointNum;i++)
    {
        double tauDes{0};
...
double PVT_Ctr_V4_Weld::sign(double in) {
    if (in>=0)
        return 1.0;
    else
        return -1.0;
...
void PVT_Ctr_V4_Weld::enablePV() {
    PV_enable.assign(jointNum,1);
}

void PVT_Ctr_V4_Weld::enablePV(int jtId) {
    PV_enable[jtId]=1;
}

void PVT_Ctr_V4_Weld::disablePV() {
    PV_enable.assign(jointNum,0);
}

void PVT_Ctr_V4_Weld::disablePV(int jtId) {
    PV_enable[jtId]=0;
}
