/*
 * PVT_Ctr_V4 implementation for speedbot_v4 robot.
 * Adapted from PVT_ctrl.cpp for OpenLoong.
 */
#include "PVT_ctrl_v4.h"

PVT_Ctr_V4::PVT_Ctr_V4(double timeStepIn, const char *jsonPath) {
...
void PVT_Ctr_V4::dataBusRead(DataBus &busIn) {
    for (int i=0;i<jointNum;i++)
    {
...
void PVT_Ctr_V4::dataBusWrite(DataBus &busIn) {
    busIn.motors_tor_out=motor_tor_out_motor;
    busIn.motors_tor_cur=motor_tor_out_link;
}

void PVT_Ctr_V4::setJointPD(double kp, double kd, const char *jointName) {
...
void PVT_Ctr_V4::calMotorsPVT(double deltaP_Lim) {
    for (int i=0;i<jointNum;i++)
    {
        double tauDes{0};
...
double PVT_Ctr_V4::sign(double in) {
    if (in>=0)
        return 1.0;
    else
        return -1.0;
...
void PVT_Ctr_V4::enablePV() {
    PV_enable.assign(jointNum,1);
}

void PVT_Ctr_V4::enablePV(int jtId) {
    PV_enable[jtId]=1;
}

void PVT_Ctr_V4::disablePV() {
    PV_enable.assign(jointNum,0);
}

void PVT_Ctr_V4::disablePV(int jtId) {
    PV_enable[jtId]=0;
}
