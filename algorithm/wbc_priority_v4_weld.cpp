/*
 * WBC_priority_V4_Weld implementation for speedbot_v4_weld robot.
 * Adapted from wbc_priority.cpp for OpenLoong.
 *
 * speedbot_v4_weld pinocchio dq ordering (model_nv=29):
 ...
 *   dq[23..28]: arm_r (shoulder_pitch, shoulder_roll, shoulder_yaw, elbow, wrist_roll, welding_gun)
 *
 * q = dq + 1 for joints (q[7..] = joints, q[0..6] = base pos + quaternion)
 */
#include "wbc_priority_v4_weld.h"
#include "iostream"
#include <algorithm>
#include <cmath>
#include <limits>
...
WBC_priority_V4_Weld::WBC_priority_V4_Weld(int model_nv_In, int QP_nvIn, int QP_ncIn, double miu_In, double dt) : QP_prob(QP_nvIn,
                                                                                                          QP_ncIn)
{
    timeStep = dt;
...
void WBC_priority_V4_Weld::setContactMiu(double miuIn)
{
    miu = std::clamp(miuIn, 0.01, 2.0);
}

void WBC_priority_V4_Weld::setJointTorqueLimits(const Eigen::VectorXd &tauLowIn, const Eigen::VectorXd &tauUppIn)
{
...
void WBC_priority_V4_Weld::dataBusRead(const DataBus &robotState)
{
...
void WBC_priority_V4_Weld::dataBusWrite(DataBus &robotState)
{
...
void WBC_priority_V4_Weld::computeTau()
{
 ...
 *   q[24..29]: arm_r (shoulder_pitch, shoulder_roll, shoulder_yaw, elbow, wrist_roll, welding_gun)
 *
 ...
 *   dq[23..28]: arm_r
 */
void WBC_priority_V4_Weld::computeDdq(Pin_KinDyn_V4_Weld &pinKinDynIn)
{
    /// -------- walk -------------
    {
        ...
        // HandTrackJoints walk: 11 DoF (arm_l 5 + arm_r 6), non-contiguous
        ...
        target_arm_q.resize(11);
        // arm_l: shoulder_pitch, shoulder_roll, shoulder_yaw, elbow, wrist_roll
        // arm_r: shoulder_pitch, shoulder_roll, shoulder_yaw, elbow, wrist_roll, welding_gun
        target_arm_q << 0.3 + 0.75*r_hip_pitch, 1.4, 0, -1.4, 0,
                       -0.3 - 0.75*l_hip_pitch, -1.4, 0, 1.4, 0, 0;

        id = kin_tasks_walk.getId("HandTrackJoints");
        kin_tasks_walk.taskLib[id].errX = Eigen::VectorXd::Zero(11);
        // arm_l at q[19..23], arm_r at q[24..29]
        kin_tasks_walk.taskLib[id].errX.block<5,1>(0,0) = target_arm_q.block<5,1>(0,0) - q.block<5, 1>(19, 0);
        kin_tasks_walk.taskLib[id].errX.block(5,0,6,1) = target_arm_q.block(5,0,6,1) - q.block(24, 0, 6, 1);
        kin_tasks_walk.taskLib[id].derrX = Eigen::VectorXd::Zero(11);
        kin_tasks_walk.taskLib[id].ddxDes = Eigen::VectorXd::Zero(11);
        kin_tasks_walk.taskLib[id].dxDes = Eigen::VectorXd::Zero(11);
        kin_tasks_walk.taskLib[id].kp = Eigen::MatrixXd::Identity(11, 11) * 200;
        kin_tasks_walk.taskLib[id].kd = Eigen::MatrixXd::Identity(11, 11) * 10;
        kin_tasks_walk.taskLib[id].J = Eigen::MatrixXd::Zero(11, model_nv);
        kin_tasks_walk.taskLib[id].J.block(0, 18, 5, 5) = Eigen::MatrixXd::Identity(5, 5); // arm_l dq[18..22]
        kin_tasks_walk.taskLib[id].J.block(5, 23, 6, 6) = Eigen::MatrixXd::Identity(6, 6); // arm_r dq[23..28]
        kin_tasks_walk.taskLib[id].dJ = Eigen::MatrixXd::Zero(11, model_nv);
        kin_tasks_walk.taskLib[id].W.diagonal() = Eigen::VectorXd::Ones(model_nv);
    }
    ...
    /// -------- stand -------------
    {
        ...
        // HipRPY: exclude arms (cols 18-22 arm_l, cols 23-28 arm_r)
        ...
        kin_tasks_stand.taskLib[id].J.block(0, 23, 3, 6).setZero();  // exclude arm_r
        ...
        // CoMTrack: exclude arms (cols 18-22 arm_l, cols 23-28 arm_r)
        ...
        kin_tasks_stand.taskLib[id].J.block(0, 23, 2, 6).setZero(); // exclude arm_r
        ...
        kin_tasks_stand.taskLib[id].J.block(2, 23, 3, 6).setZero(); // exclude arm_r
        kin_tasks_stand.taskLib[id].dJ = Eigen::MatrixXd::Zero(5, model_nv);
        kin_tasks_stand.taskLib[id].W.diagonal() = Eigen::VectorXd::Ones(model_nv);

        // HandTrackJoints stand: 11 DoF (arm_l 5 + arm_r 6)
        Eigen::VectorXd target_arm_q;
        target_arm_q.resize(11);
        target_arm_q << 0.3, 1.4, 0, -1.4, 0, -0.3, -1.4, 0, 1.4, 0, 0;
        if (qIniDes.size() >= 30)
        {
            target_arm_q.block<5, 1>(0, 0) = qIniDes.block<5, 1>(19, 0);
            target_arm_q.block(5, 0, 6, 1) = qIniDes.block(24, 0, 6, 1);
        }

        id = kin_tasks_stand.getId("HandTrackJoints");
        kin_tasks_stand.taskLib[id].errX = Eigen::VectorXd::Zero(11);
        kin_tasks_stand.taskLib[id].errX.block<5,1>(0,0) = target_arm_q.block<5,1>(0,0) - q.block<5, 1>(19, 0);
        kin_tasks_stand.taskLib[id].errX.block(5,0,6,1) = target_arm_q.block(5,0,6,1) - q.block(24, 0, 6, 1);
        kin_tasks_stand.taskLib[id].derrX = Eigen::VectorXd::Zero(11);
        kin_tasks_stand.taskLib[id].ddxDes = Eigen::VectorXd::Zero(11);
        kin_tasks_stand.taskLib[id].dxDes = Eigen::VectorXd::Zero(11);
        kin_tasks_stand.taskLib[id].kp = Eigen::MatrixXd::Identity(11, 11) * 2000;
        kin_tasks_stand.taskLib[id].kd = Eigen::MatrixXd::Identity(11, 11) * 100;
        kin_tasks_stand.taskLib[id].J = Eigen::MatrixXd::Zero(11, model_nv);
        kin_tasks_stand.taskLib[id].J.block(0, 18, 5, 5) = Eigen::MatrixXd::Identity(5, 5); // arm_l
        kin_tasks_stand.taskLib[id].J.block(5, 23, 6, 6) = Eigen::MatrixXd::Identity(6, 6); // arm_r
        kin_tasks_stand.taskLib[id].dJ = Eigen::MatrixXd::Zero(11, model_nv);
        kin_tasks_stand.taskLib[id].W.diagonal() = Eigen::VectorXd::Ones(model_nv);
    ...
    /// -------- weld -------------
    {
        ...
        kin_tasks_weld.taskLib[id].J.block(0, 23, 2, 6).setZero();
        kin_tasks_weld.taskLib[id].J.block(2, 18, 3, 5).setZero();
        kin_tasks_weld.taskLib[id].J.block(2, 23, 3, 6).setZero();
        ...
        kin_tasks_weld.taskLib[id].errX = Eigen::VectorXd::Zero(6);
        Eigen::Vector3d weldPosErr = weld_tcp_pos_des_W - hd_r_pos_cur_W;
        Eigen::Matrix3d weldDesRotTask = weld_tcp_rot_des_W;
        Eigen::Vector3d weldRotErr = diffRot(hd_r_rot_cur_W, weldDesRotTask);
        const double weldPosErrNorm = weldPosErr.norm();
        const double weldRotErrNorm = weldRotErr.norm();
        const bool isWeldPrepare = (motionStateCur == DataBus::WeldPrepare);
        const double maxWeldPosErrForTask = isWeldPrepare ? 0.025 : 0.025;
        const double maxWeldRotErrForTask = isWeldPrepare ? 0.35 : 0.30;
        if (weldPosErrNorm > maxWeldPosErrForTask)
        {
            weldPosErr *= maxWeldPosErrForTask / weldPosErrNorm;
        }
        if (weldRotErrNorm > maxWeldRotErrForTask)
        {
            weldRotErr *= maxWeldRotErrForTask / weldRotErrNorm;
        }
        ...
        kin_tasks_weld.taskLib[id].errX.block<3, 1>(3, 0) = weldRotErr;
        Eigen::MatrixXd rightHandJ = Eigen::MatrixXd::Zero(6, model_nv);
        Eigen::MatrixXd rightHanddJ = Eigen::MatrixXd::Zero(6, model_nv);
        rightHandJ.block(0, 23, 6, 6) = J_hd_r.block(0, 23, 6, 6);
        rightHanddJ.block(0, 23, 6, 6) = dJ_hd_r.block(0, 23, 6, 6);
        Eigen::Matrix<double, 6, 1> handVel = rightHandJ * dq;
        kin_tasks_weld.taskLib[id].derrX = Eigen::VectorXd::Zero(6);
        kin_tasks_weld.taskLib[id].derrX.block<3, 1>(0, 0) =
            handTaskScale * weld_tcp_linear_vel_des_W - handVel.block<3, 1>(0, 0);
        kin_tasks_weld.taskLib[id].derrX.block<3, 1>(3, 0) =
            handTaskScale * weld_tcp_angular_vel_des_W - handVel.block<3, 1>(3, 0);
        kin_tasks_weld.taskLib[id].ddxDes = Eigen::VectorXd::Zero(6);
        kin_tasks_weld.taskLib[id].ddxDes.block<3, 1>(0, 0) = handTaskScale * weld_tcp_linear_acc_des_W;
        kin_tasks_weld.taskLib[id].ddxDes.block<3, 1>(3, 0) = handTaskScale * weld_tcp_angular_acc_des_W;
        kin_tasks_weld.taskLib[id].dxDes = Eigen::VectorXd::Zero(6);
        kin_tasks_weld.taskLib[id].dxDes.block<3, 1>(0, 0) = handTaskScale * weld_tcp_linear_vel_des_W;
        kin_tasks_weld.taskLib[id].dxDes.block<3, 1>(3, 0) = handTaskScale * weld_tcp_angular_vel_des_W;
        kin_tasks_weld.taskLib[id].kp = Eigen::MatrixXd::Identity(6, 6) * (cfg_weld_hand_kp * handTaskScale);
        kin_tasks_weld.taskLib[id].kd = Eigen::MatrixXd::Identity(6, 6) * (cfg_weld_hand_kd * handTaskScale);
        kin_tasks_weld.taskLib[id].kp.block<3, 3>(3, 3) *= 0.8;
        kin_tasks_weld.taskLib[id].kd.block<3, 3>(3, 3) *= 0.8;
        kin_tasks_weld.taskLib[id].J = rightHandJ;
        kin_tasks_weld.taskLib[id].dJ = rightHanddJ;
        kin_tasks_weld.taskLib[id].W.diagonal() = Eigen::VectorXd::Ones(model_nv);

        id = kin_tasks_weld.getId("RightArmRecover");
        Eigen::VectorXd target_right_arm = Eigen::VectorXd::Zero(6);
        target_right_arm << -0.3, -1.4, 0.0, 1.4, 0.0, 0.0;
        if (qIniDes.size() >= 30)
        {
            target_right_arm = qIniDes.block(24, 0, 6, 1);
        }
        const double rightArmRecoverScale =
            (motionStateCur == DataBus::WeldRecover) ? std::clamp(weld_recover_phase, 0.0, 1.0) : 0.0;
        kin_tasks_weld.taskLib[id].errX = target_right_arm - q.block(24, 0, 6, 1);
        kin_tasks_weld.taskLib[id].derrX = -dq.block(23, 0, 6, 1);
        kin_tasks_weld.taskLib[id].ddxDes = Eigen::VectorXd::Zero(6);
        kin_tasks_weld.taskLib[id].dxDes = Eigen::VectorXd::Zero(6);
        kin_tasks_weld.taskLib[id].kp = Eigen::MatrixXd::Identity(6, 6) *
                                        (cfg_weld_right_arm_recover_kp * rightArmRecoverScale);
        kin_tasks_weld.taskLib[id].kd = Eigen::MatrixXd::Identity(6, 6) *
                                        (cfg_weld_right_arm_recover_kd * rightArmRecoverScale);
        kin_tasks_weld.taskLib[id].J = Eigen::MatrixXd::Zero(6, model_nv);
        if (rightArmRecoverScale > 1.0e-6)
        {
            kin_tasks_weld.taskLib[id].J.block(0, 23, 6, 6) = Eigen::MatrixXd::Identity(6, 6);
        }
        else
        {
        ...
        kin_tasks_weld.taskLib[id].dJ = Eigen::MatrixXd::Zero(6, model_nv);
        kin_tasks_weld.taskLib[id].W.diagonal() = Eigen::VectorXd::Ones(model_nv);
...
void WBC_priority_V4_Weld::copy_Eigen_to_real_t(qpOASES::real_t *target, const Eigen::MatrixXd &source, int nRows, int nCols)
{
    int count = 0;
...
void WBC_priority_V4_Weld::setQini(const Eigen::VectorXd &qIniDesIn, const Eigen::VectorXd &qIniCurIn)
{
    qIniDes = qIniDesIn;
    qIniCur = qIniCurIn;
}
