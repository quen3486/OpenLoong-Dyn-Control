...
ROS_STATE_PUB="${REPO_ROOT}/sim_interface/ROS2_state_pub_v4_leg.cpp"
...
RVIZ_CFG="${REPO_ROOT}/tools/real_robot/speedbot_v4_leg.rviz"
...
check_sim_ros2_state_pub_config() {
  local desc="v4_leg保留仿真ROS2状态发布开关"
  if python3 - "$CFG_FILE" <<'PY'
import json
import re
import sys
...
text = re.sub(r"//.*", "", text)
cfg = json.loads(text)
value = cfg.get("sim_enable_ros2_state_pub")
if not isinstance(value, bool):
    raise SystemExit("sim_enable_ros2_state_pub is missing or not boolean")
PY
  then
    echo "[PASS] ${desc}"
    pass_count=$((pass_count + 1))
...
check_real_joint_state_sample() {
  ...
  if python3 - "$REAL_JOINT_STATE_SAMPLE" "$ROS_STATE_PUB" <<'PY'
import ast
import re
import sys

sample_path, state_pub_path = sys.argv[1:3]
expected_real_order = [
    "left_hip_roll_joint",
    "left_hip_yaw_joint",
...
expected_internal_from_real = [0, 1, 2, 5, 4, 7, 3, 9, 6, 10, 8, 11]
...
with open(state_pub_path, "r", encoding="utf-8") as f:
    cpp = f.read()
order_match = re.search(r"kJointNamesRosOrder\s*=\s*\{(.*?)\};", cpp, flags=re.S)
map_match = re.search(r"kRosToInternalIndex\s*=\s*\{(.*?)\};", cpp, flags=re.S)
if not order_match or not map_match:
    raise SystemExit("missing ROS state publisher order arrays")
cpp_order = re.findall(r'"([^"]+)"', order_match.group(1))
cpp_map = ast.literal_eval("[" + map_match.group(1).replace("\n", " ") + "]")
if cpp_order != expected_real_order:
    raise SystemExit(f"ROS2_StatePub_V4_Leg order mismatch: {cpp_order}")
if cpp_map != expected_internal_from_real:
    raise SystemExit(f"ROS2_StatePub_V4_Leg remap mismatch: {cpp_map}")
PY
  then
    echo "[PASS] ${desc}"
    pass_count=$((pass_count + 1))
...
check_pattern "$RVIZ_SCRIPT" 'kill -KILL' "RViz脚本对未退出旧RSP做强制兜底清理"
