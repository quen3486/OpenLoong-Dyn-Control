/*
 * MJ_Interface implementation for speedbot_v4 robot.
 * Adapted from MJ_interface.cpp for OpenLoong.
 */
#include "MJ_interface_v4.h"

MJ_Interface_V4::MJ_Interface_V4(mjModel *mj_modelIn, mjData *mj_dataIn)
{
    mj_model = mj_modelIn;
    mj_data = mj_dataIn;
...
void MJ_Interface_V4::updateSensorValues()
{
    for (int i = 0; i < jointNum; i++)
    {
...
void MJ_Interface_V4::setMotorsTorque(std::vector<double> &tauIn)
{
    for (int i = 0; i < jointNum; i++)
        mj_data->ctrl[jntId_dctl[i]] = tauIn.at(i);
}

void MJ_Interface_V4::dataBusWrite(DataBus &busIn)
{
