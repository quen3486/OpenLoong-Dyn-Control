/*
 * MJ_Interface implementation for speedbot_v4_weld robot.
 * Adapted from MJ_interface.cpp for OpenLoong.
 */
#include "MJ_interface_v4_weld.h"

MJ_Interface_V4_Weld::MJ_Interface_V4_Weld(mjModel *mj_modelIn, mjData *mj_dataIn)
{
    mj_model = mj_modelIn;
    mj_data = mj_dataIn;
...
void MJ_Interface_V4_Weld::updateSensorValues()
{
    for (int i = 0; i < jointNum; i++)
    {
...
void MJ_Interface_V4_Weld::setMotorsPos(std::vector<double> &posIn)
{
    for (int i = 0; i < jointNum; i++)
    {
        mj_data->ctrl[jntId_dctl[i]] = posIn.at(i);
    }
}

void MJ_Interface_V4_Weld::dataBusWrite(DataBus &busIn)
{
